...
bool chratos_qt::eventloop_processor::event (QEvent * event_a)
{
	assert (dynamic_cast<chratos_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<chratos_qt::eventloop_event *> (event_a)->action ();
	return true;
}

chratos_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

chratos_qt::self_pane::self_pane (chratos_qt::wallet & wallet_a, chratos::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
self_layout (new QHBoxLayout),
self_window (new QWidget),
your_account_label (new QLabel ("Your Chratos account:")),
...
void chratos_qt::self_pane::refresh_balance ()
{
...
chratos_qt::history::history (chratos::ledger & ledger_a, chratos::account const & account_a, chratos_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
chratos_qt::block_viewer::block_viewer (chratos_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
chratos_qt::account_viewer::account_viewer (chratos_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (CHR): ") + wallet.format_balance (balance.first));
			if (!balance.second.is_zero ())
			{
				final_text += "\nPending: " + wallet.format_balance (balance.second);
			}
...
void chratos_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
chratos_qt::status::status (chratos_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (chratos_qt::status_types::nominal);
	set_text ();
}

void chratos_qt::status::erase (chratos_qt::status_types status_a)
{
	assert (status_a != chratos_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void chratos_qt::status::insert (chratos_qt::status_types status_a)
{
	assert (status_a != chratos_qt::status_types::nominal);
...
std::string chratos_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case chratos_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case chratos_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case chratos_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case chratos_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case chratos_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case chratos_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case chratos_qt::status_types::nominal:
...
std::string chratos_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
chratos_qt::wallet::wallet (QApplication & application_a, chratos_qt::eventloop_processor & processor_a, chratos::node & node_a, std::shared_ptr<chratos::wallet> wallet_a, chratos::account & account_a) :
rendering_ratio (chratos::Mchr_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
active_status (*this)
{
	...
	send_account->setPlaceholderText (chratos::zero_key.pub.to_account ().c_str ());
...
void chratos_qt::wallet::start ()
{
	std::weak_ptr<chratos_qt::wallet> this_w (shared_from_this ());
	...
	QObject::connect (send_blocks_send, &QPushButton::released, [this_w]() {
		if (auto this_l = this_w.lock ())
		{
			...
			{
				chratos::uint128_t actual (amount.number () * this_l->rendering_ratio);
						...
						if (actual <= balance)
						{
							...
							if (this_l->wallet_m->store.valid_password (transaction))
							{
								...
								this_l->node.background ([this_w, account_l, actual]() {
									if (auto this_l = this_w.lock ())
									{
										this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w](std::shared_ptr<chratos::block> block_a) {
											if (auto this_l = this_w.lock ())
											{
	...
	node.observers.blocks.add ([this_w](std::shared_ptr<chratos::block> block_a, chratos::account const & account_a, chratos::uint128_t const & amount_a, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w](chratos::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (chratos_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (chratos_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w](chratos::endpoint const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (chratos_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (chratos_qt::status_types::synchronizing);
					}
				}
			}));
		}
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (chratos_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (chratos_qt::status_types::working);
					}
				}
			}));
		}
...
void chratos_qt::wallet::update_connected ()
{
	if (node.peers.empty ())
	{
		active_status.insert (chratos_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (chratos_qt::status_types::disconnected);
	}
}
...
void chratos_qt::wallet::change_rendering_ratio (chratos::uint128_t const & rendering_ratio_a)
{
...
std::string chratos_qt::wallet::format_balance (chratos::uint128_t const & balance) const
{
	auto balance_str = chratos::amount (balance).format_balance (rendering_ratio, 0, false);
	auto unit = std::string ("CHR");
	if (rendering_ratio == chratos::kchr_ratio)
	{
		unit = std::string ("kchr");
	}
	else if (rendering_ratio == chratos::chr_ratio)
	{
		unit = std::string ("chr");
	}
...
void chratos_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void chratos_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

chratos_qt::settings::settings (chratos_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
wallet (wallet_a)
{
	...
	new_representative->setPlaceholderText (chratos::zero_key.pub.to_account ().c_str ());
...
void chratos_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void chratos_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (chratos_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (chratos_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (chratos_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (chratos_qt::status_types::vulnerable);
	}
}

chratos_qt::advanced_actions::advanced_actions (chratos_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
  ...
  layout->addWidget (claim_dividends);
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this]() {
		...
		auto error (chratos::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void chratos_qt::advanced_actions::refresh_peers ()
{
...
void chratos_qt::advanced_actions::refresh_ledger ()
{
	...
	{
		QList<QStandardItem *> items;
		items.push_back (new QStandardItem (QString (chratos::block_hash (i->first).to_account ().c_str ())));
		chratos::account_info info (i->second);
		std::string balance;
		chratos::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void chratos_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

chratos_qt::block_entry::block_entry (chratos_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this]() {
		...
		try
		{
			...
			auto block_l (chratos::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
...
chratos_qt::block_creation::block_creation (chratos_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void chratos_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void chratos_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void chratos_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void chratos_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void chratos_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
...
void chratos_qt::block_creation::create_receive ()
{
	chratos::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				chratos::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending_get (transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							auto rep_block (wallet.node.store.block_get (transaction, info.rep_block));
							assert (rep_block != nullptr);
							chratos::state_block receive (pending_key.account, info.head, rep_block->representative (), info.balance.number () + pending.amount.number (), source_l, pending.dividend, key, pending_key.account, 0);
...
void chratos_qt::block_creation::create_change ()
{
	chratos::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					chratos::state_block change (account_l, info.head, representative_l, info.balance, 0, info.dividend_block, key, account_l, 0);
...
void chratos_qt::block_creation::create_open ()
{
	chratos::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending_get (transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								chratos::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, pending.dividend, key, pending_key.account, 0);
