...
{
	QMessageBox message (QMessageBox::Critical, "Error starting Chratos", message_a.c_str ());
...
bool update_config (qt_wallet_config & config_a, boost::filesystem::path const & config_path_a, std::fstream & config_file_a)
{
	...
	auto error (false);
	if (!chratos::fetch_object (config_a, config_path_a, config_file_a))
	{
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path)
{
	...
	if (!error)
	{
		...
		std::shared_ptr<chratos_qt::wallet> gui;
		chratos::set_application_icon (application);
		...
		if (!init.error ())
		{
			...
			if (config.account.is_zero () || !wallet->exists (config.account))
			{
				...
				{
					chratos::uint256_union account (existing->first);
			...
			chratos::thread_runner runner (service, node->config.io_threads);
			QObject::connect (&application, &QApplication::aboutToQuit, [&]() {
				rpc->stop ();
				node->stop ();
			});
			application.postEvent (&processor, new chratos_qt::eventloop_event ([&]() {
				gui = std::make_shared<chratos_qt::wallet> (application, processor, *node, wallet, config.account);
				splash->close ();
				gui->start ();
		...
		if (ec == chratos::error_cli::unknown_command)
		{
			...
			else
			{
				try
				{
					...
					{
						data_path = chratos::working_path ();
					}
