...
TEST (wallet, construction)
{
	...
	auto wallet_l (system.nodes[0]->wallets.create (chratos::uint256_union ()));
	auto key (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key));
	wallet->start ();
...
TEST (wallet, status)
{
	...
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub));
	wallet->start ();
	auto wallet_has = [wallet](chratos_qt::status_types status_ty) {
	...
	system.nodes[0]->peers.insert (chratos::endpoint (boost::asio::ip::address_v6::loopback (), 10000), 0);
	...
	ASSERT_FALSE (wallet_has (chratos_qt::status_types::synchronizing));
	system.deadline_set (25s);
	while (!wallet_has (chratos_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (chratos_qt::status_types::disconnected));
}
...
TEST (wallet, startup_balance)
{
	...
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub));
	wallet->start ();
	ASSERT_EQ ("Balance: 0 CHR", wallet->self.balance_label->text ().toStdString ());
}
...
TEST (wallet, select_account)
{
	...
	chratos::system system (24000, 1);
	auto wallet_l (system.nodes[0]->wallets.create (chratos::uint256_union ()));
	chratos::public_key key1 (wallet_l->deterministic_insert ());
	chratos::public_key key2 (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key1));
	wallet->start ();
...
TEST (wallet, password_change)
{
	...
	system.wallet (0)->insert_adhoc (chratos::keypair ().prv);
	{
		auto transaction (system.nodes[0]->store.tx_begin ());
		account = system.account (transaction, 0);
	}
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->settings_button, Qt::LeftButton);
	{
		auto transaction (system.nodes[0]->store.tx_begin ());
...
TEST (wallet, send)
{
	...
	chratos::public_key key1 (system.wallet (1)->insert_adhoc (chratos::keypair ().prv));
	auto account (chratos::test_genesis_key.pub);
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->send_blocks, Qt::LeftButton);
	...
	chratos::uint128_t amount (wallet->node.balance (key1));
...
TEST (wallet, process_block)
{
	...
	chratos::system system (24000, 1);
	chratos::account account;
	chratos::block_hash latest (system.nodes[0]->latest (chratos::genesis_account));
	system.wallet (0)->insert_adhoc (chratos::keypair ().prv);
	{
		auto transaction (system.nodes[0]->store.tx_begin ());
		account = system.account (transaction, 0);
	...
	chratos::send_block send (latest, key1.pub, 0, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (latest));
	std::string previous;
...
<<<<<<< upmaster
	QTest::keyClicks (wallet->block_creation.account, chratos::test_genesis_key.pub.to_account ().c_str ());
	...
	bool error (false);
	chratos::state_block send (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (chratos::process_result::progress, system.nodes[0]->process (send).code);
	ASSERT_EQ (chratos::process_result::old, system.nodes[0]->process (send).code);
}
...
TEST (wallet, create_open_receive)
{
	...
	chratos::system system (24000, 1);
	system.wallet (0)->insert_adhoc (chratos::test_genesis_key.prv);
	system.wallet (0)->send_action (chratos::test_genesis_key.pub, key.pub, 100);
	chratos::block_hash latest1 (system.nodes[0]->latest (chratos::test_genesis_key.pub));
	system.wallet (0)->send_action (chratos::test_genesis_key.pub, key.pub, 100);
	chratos::block_hash latest2 (system.nodes[0]->latest (chratos::test_genesis_key.pub));
	...
	QTest::keyClicks (wallet->block_creation.representative, chratos::test_genesis_key.pub.to_account ().c_str ());
	QTest::mouseClick (wallet->block_creation.create, Qt::LeftButton);
	...
	bool error (false);
	chratos::state_block open (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (chratos::process_result::progress, system.nodes[0]->process (open).code);
	ASSERT_EQ (chratos::process_result::old, system.nodes[0]->process (open).code);
	...
	bool error2 (false);
	chratos::state_block receive (error2, tree2);
	ASSERT_FALSE (error2);
	ASSERT_EQ (chratos::process_result::progress, system.nodes[0]->process (receive).code);
	ASSERT_EQ (chratos::process_result::old, system.nodes[0]->process (receive).code);
}
...
TEST (wallet, create_change)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	chratos::state_block change (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (chratos::process_result::progress, system.nodes[0]->process (change).code);
	ASSERT_EQ (chratos::process_result::old, system.nodes[0]->process (change).code);
}
...
TEST (history, short_text)
{
	...
	chratos::ledger ledger (store, system.nodes[0]->stats);
	{
		auto transaction (store.tx_begin (true));
		store.initialize (transaction, genesis);
		chratos::keypair key;
		chratos::send_block send (ledger.latest (transaction, chratos::test_genesis_key.pub), chratos::test_genesis_key.pub, 0, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0);
		...
		chratos::change_block change (receive.hash (), key.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0);
		ASSERT_EQ (chratos::process_result::progress, ledger.process (transaction, change).code);
	}
	chratos_qt::history history (ledger, chratos::test_genesis_key.pub, *wallet);
	history.refresh ();
...
TEST (wallet, startup_work)
{
	...
	{
		auto transaction (system.nodes[0]->store.tx_begin ());
		ASSERT_TRUE (wallet->wallet_m->store.work_get (transaction, chratos::test_genesis_key.pub, work1));
	}
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	while (again)
	{
		ASSERT_NO_ERROR (system.poll ());
		auto transaction (system.nodes[0]->store.tx_begin ());
		again = wallet->wallet_m->store.work_get (transaction, chratos::test_genesis_key.pub, work1);
	}
}
...
TEST (wallet, import)
{
	...
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, republish)
{
	...
	{
		auto transaction (system.nodes[0]->store.tx_begin (true));
		chratos::send_block block (system.nodes[0]->ledger.latest (transaction, chratos::test_genesis_key.pub), key.pub, 0, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0);
		hash = block.hash ();
	...
	ASSERT_FALSE (system.nodes[1]->balance (chratos::test_genesis_key.pub).is_zero ());
	system.deadline_set (10s);
	while (system.nodes[1]->balance (chratos::test_genesis_key.pub).is_zero ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	chratos::keypair key1;
	system.wallet (0)->insert_adhoc (key1.prv);
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	ASSERT_EQ (wallet->accounts.window, wallet->main_stack->currentWidget ());
	QTest::keyClicks (wallet->accounts.account_key_line, chratos::test_genesis_key.prv.data.to_string ().c_str ());
	QTest::mouseClick (wallet->accounts.account_key_button, Qt::LeftButton);
	ASSERT_EQ (1, wallet->accounts.model->rowCount ());
...
TEST (wallet, change_seed)
{
	...
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, seed_work_generation)
{
	...
	chratos::system system (24000, 1);
	auto key1 (system.wallet (0)->deterministic_insert ());
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	auto transaction (system.wallet (0)->wallets.tx_begin ());
	ASSERT_FALSE (chratos::work_validate (system.nodes[0]->ledger.latest_root (transaction, key1), work));
}
...
TEST (wallet, DISABLED_synchronizing)
{
	...
	auto wallet (std::make_shared<chratos_qt::wallet> (*test_application, processor, *system0.nodes[0], system0.wallet (0), key1));
	wallet->start ();
	{
		auto transaction (system1.nodes[0]->store.tx_begin (true));
		auto latest (system1.nodes[0]->ledger.latest (transaction, chratos::genesis_account));
		chratos::send_block send (latest, key1, 0, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system1.work.generate (latest));
		system1.nodes[0]->ledger.process (transaction, send);
	}
	ASSERT_EQ (0, wallet->active_status.active.count (chratos_qt::status_types::synchronizing));
	system0.nodes[0]->bootstrap_initiator.bootstrap (system1.nodes[0]->network.endpoint ());
	system1.deadline_set (10s);
	while (wallet->active_status.active.count (chratos_qt::status_types::synchronizing) == 0)
	{
	...
	while (wallet->active_status.active.count (chratos_qt::status_types::synchronizing) == 1)
	{
		ASSERT_NO_ERROR (system0.poll ());
		ASSERT_NO_ERROR (system1.poll ());
		test_application->processEvents ();
