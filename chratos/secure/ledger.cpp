...
public:
	...
	void receive_block (chratos::receive_block const & block_a) override
	{
		...
		ledger.store.block_del (transaction, hash);
		ledger.store.pending_put (transaction, chratos::pending_key (destination_account, block_a.hashables.source), { source_account, amount, chratos::epoch::epoch_0 });
		ledger.store.frontier_del (transaction, hash);
	...
	void state_block (chratos::state_block const & block_a) override
	{
		...
		else if (!block_a.hashables.link.is_zero () && block_a.hashables.link != ledger.epoch_link)
		{
			...
			chratos::pending_info pending_info (ledger.account (transaction, block_a.hashables.link), block_a.hashables.balance.number () - balance, source_version);
		...
		if (result.code == chratos::process_result::progress)
		{
			...
			if (result.code == chratos::process_result::progress)
			{
				...
				if (result.code == chratos::process_result::progress)
				{
					...
					{
						chratos::pending_key key (block_a.hashables.link, hash);
						chratos::pending_info info (block_a.hashables.account, result.amount.number (), epoch);
				...
				if (result.code == chratos::process_result::progress)
				{
					...
					if (result.code == chratos::process_result::progress)
					{
						...
						if (result.code == chratos::process_result::progress)
						{
							...
							ledger.store.pending_put (transaction, chratos::pending_key (block_a.hashables.destination, hash), { account, amount, chratos::epoch::epoch_0 });
							ledger.store.frontier_del (transaction, block_a.hashables.previous);
							ledger.store.frontier_put (transaction, hash, account);
							result.account = account;
