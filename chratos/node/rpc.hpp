...
public:
	...
	virtual void accept ();
	void stop ();
	void observer_action (chratos::account const &);
	boost::asio::ip::tcp::acceptor acceptor;
	std::mutex mutex;
	std::unordered_map<chratos::account, std::shared_ptr<chratos::payment_observer>> payment_observers;
	chratos::rpc_config config;
	chratos::node & node;
	bool on;
	static uint16_t const rpc_port = chratos::chratos_network == chratos::chratos_networks::chratos_live_network ? 9126 : 45000;
};
class rpc_connection : public std::enable_shared_from_this<chratos::rpc_connection>
{
public:
...
class payment_observer : public std::enable_shared_from_this<chratos::payment_observer>
{
public:
	payment_observer (std::function<void(boost::property_tree::ptree const &)> const &, chratos::rpc &, chratos::account const &, chratos::amount const &);
	~payment_observer ();
	void start (uint64_t);
	void observe ();
	void timeout ();
	void complete (chratos::payment_status);
	std::mutex mutex;
...
class rpc_handler : public std::enable_shared_from_this<chratos::rpc_handler>
{
public:
	rpc_handler (chratos::node &, chratos::rpc &, std::string const &, std::string const &, std::function<void(boost::property_tree::ptree const &)> const &);
	...
	std::shared_ptr<chratos::wallet> wallet_impl ();
	chratos::account account_impl (std::string = "");
	chratos::amount amount_impl ();
	chratos::block_hash hash_impl (std::string = "hash");
	chratos::amount threshold_optional_impl ();
...
std::unique_ptr<chratos::rpc> get_rpc (boost::asio::io_service & service_a, chratos::node & node_a, chratos::rpc_config const & config_a);
}
