...
chratos::uint256_union chratos::wallet_store::check (chratos::transaction const & transaction_a)
{
	chratos::wallet_value value (entry_get_raw (transaction_a, chratos::wallet_store::check_special));
	return value.key;
}

chratos::uint256_union chratos::wallet_store::salt (chratos::transaction const & transaction_a)
{
	chratos::wallet_value value (entry_get_raw (transaction_a, chratos::wallet_store::salt_special));
	return value.key;
}

void chratos::wallet_store::wallet_key (chratos::raw_key & prv_a, chratos::transaction const & transaction_a)
{
...
void chratos::wallet_store::seed (chratos::raw_key & prv_a, chratos::transaction const & transaction_a)
{
...
void chratos::wallet_store::seed_set (chratos::transaction const & transaction_a, chratos::raw_key const & prv_a)
{
	chratos::raw_key password_l;
	wallet_key (password_l, transaction_a);
...
chratos::public_key chratos::wallet_store::deterministic_insert (chratos::transaction const & transaction_a)
{
...
void chratos::wallet_store::deterministic_key (chratos::raw_key & prv_a, chratos::transaction const & transaction_a, uint32_t index_a)
{
...
uint32_t chratos::wallet_store::deterministic_index_get (chratos::transaction const & transaction_a)
{
...
void chratos::wallet_store::deterministic_index_set (chratos::transaction const & transaction_a, uint32_t index_a)
{
...
void chratos::wallet_store::deterministic_clear (chratos::transaction const & transaction_a)
{
...
bool chratos::wallet_store::valid_password (chratos::transaction const & transaction_a)
{
...
bool chratos::wallet_store::attempt_password (chratos::transaction const & transaction_a, std::string const & password_a)
{
	bool result = false;
	{
		std::lock_guard<std::recursive_mutex> lock (mutex);
...
bool chratos::wallet_store::rekey (chratos::transaction const & transaction_a, std::string const & password_a)
{
	std::lock_guard<std::recursive_mutex> lock (mutex);
	bool result (false);
...
void chratos::wallet_store::derive_key (chratos::raw_key & prv_a, chratos::transaction const & transaction_a, std::string const & password_a)
{
...
kdf (kdf_a)
{
	...
	{
		MDB_val junk;
		assert (mdb_get (tx (transaction_a), handle, chratos::mdb_val (version_special), &junk) == MDB_NOTFOUND);
		...
		init_a |= mdb_get (tx (transaction_a), handle, chratos::mdb_val (version_special), &junk) != 0;
		init_a |= mdb_get (tx (transaction_a), handle, chratos::mdb_val (wallet_key_special), &junk) != 0;
		init_a |= mdb_get (tx (transaction_a), handle, chratos::mdb_val (salt_special), &junk) != 0;
		init_a |= mdb_get (tx (transaction_a), handle, chratos::mdb_val (check_special), &junk) != 0;
		init_a |= mdb_get (tx (transaction_a), handle, chratos::mdb_val (representative_special), &junk) != 0;
		chratos::raw_key key;
		key.data.clear ();
...
kdf (kdf_a)
{
	...
	if (!init_a)
	{
		...
		version_status = mdb_get (tx (transaction_a), handle, chratos::mdb_val (version_special), &version_value);
...
std::vector<chratos::account> chratos::wallet_store::accounts (chratos::transaction const & transaction_a)
{
...
void chratos::wallet_store::initialize (chratos::transaction const & transaction_a, bool & init_a, std::string const & path_a)
{
...
bool chratos::wallet_store::is_representative (chratos::transaction const & transaction_a)
{
	return exists (transaction_a, representative (transaction_a));
}

void chratos::wallet_store::representative_set (chratos::transaction const & transaction_a, chratos::account const & representative_a)
{
	entry_put_raw (transaction_a, chratos::wallet_store::representative_special, chratos::wallet_value (representative_a, 0));
}

chratos::account chratos::wallet_store::representative (chratos::transaction const & transaction_a)
{
	chratos::wallet_value value (entry_get_raw (transaction_a, chratos::wallet_store::representative_special));
	return value.key;
}

chratos::public_key chratos::wallet_store::insert_adhoc (chratos::transaction const & transaction_a, chratos::raw_key const & prv)
{
	assert (valid_password (transaction_a));
...
void chratos::wallet_store::insert_watch (chratos::transaction const & transaction_a, chratos::public_key const & pub)
{
	entry_put_raw (transaction_a, pub, chratos::wallet_value (chratos::uint256_union (0), 0));
}

void chratos::wallet_store::erase (chratos::transaction const & transaction_a, chratos::public_key const & pub)
{
	auto status (mdb_del (tx (transaction_a), handle, chratos::mdb_val (pub), nullptr));
	assert (status == 0);
}

chratos::wallet_value chratos::wallet_store::entry_get_raw (chratos::transaction const & transaction_a, chratos::public_key const & pub_a)
{
	chratos::wallet_value result;
	chratos::mdb_val value;
	auto status (mdb_get (tx (transaction_a), handle, chratos::mdb_val (pub_a), value));
	if (status == 0)
	{
...
void chratos::wallet_store::entry_put_raw (chratos::transaction const & transaction_a, chratos::public_key const & pub_a, chratos::wallet_value const & entry_a)
{
	auto status (mdb_put (tx (transaction_a), handle, chratos::mdb_val (pub_a), entry_a.val (), 0));
	assert (status == 0);
}
...
bool chratos::wallet_store::fetch (chratos::transaction const & transaction_a, chratos::public_key const & pub, chratos::raw_key & prv)
{
	auto result (false);
	if (valid_password (transaction_a))
	{
...
bool chratos::wallet_store::exists (chratos::transaction const & transaction_a, chratos::public_key const & pub)
{
	return !pub.is_zero () && find (transaction_a, pub) != end ();
}

void chratos::wallet_store::serialize_json (chratos::transaction const & transaction_a, std::string & string_a)
{
...
void chratos::wallet_store::write_backup (chratos::transaction const & transaction_a, boost::filesystem::path const & path_a)
{
	...
	if (!backup_file.fail ())
	{
		...
		chratos::set_secure_perm_file (path_a, ec);
...
bool chratos::wallet_store::move (chratos::transaction const & transaction_a, chratos::wallet_store & other_a, std::vector<chratos::public_key> const & keys)
{
	assert (valid_password (transaction_a));
...
bool chratos::wallet_store::import (chratos::transaction const & transaction_a, chratos::wallet_store & other_a)
{
	assert (valid_password (transaction_a));
	assert (other_a.valid_password (transaction_a));
	auto result (false);
	for (auto i (other_a.begin (transaction_a)), n (end ()); i != n; ++i)
	{
		...
		if (!result)
		{
			...
			{
				insert_watch (transaction_a, chratos::uint256_union (i->first));
			}
...
bool chratos::wallet_store::work_get (chratos::transaction const & transaction_a, chratos::public_key const & pub_a, uint64_t & work_a)
{
	auto result (false);
...
void chratos::wallet_store::work_put (chratos::transaction const & transaction_a, chratos::public_key const & pub_a, uint64_t work_a)
{
	auto entry (entry_get_raw (transaction_a, pub_a));
...
unsigned chratos::wallet_store::version (chratos::transaction const & transaction_a)
{
...
void chratos::wallet_store::version_put (chratos::transaction const & transaction_a, unsigned version_a)
{
...
void chratos::wallet_store::upgrade_v1_v2 (chratos::transaction const & transaction_a)
{
	assert (version (transaction_a) == 1);
	chratos::raw_key zero_password;
	chratos::wallet_value value (entry_get_raw (transaction_a, chratos::wallet_store::wallet_key_special));
	chratos::raw_key kdf;
	kdf.data.clear ();
	...
	for (auto i (begin (transaction_a)), n (end ()); i != n; ++i)
	{
		...
		if (fetch (transaction_a, key, prv))
		{
			...
			chratos::wallet_value data (entry_get_raw (transaction_a, key));
...
void chratos::wallet_store::upgrade_v2_v3 (chratos::transaction const & transaction_a)
{
...
void chratos::wallet_store::upgrade_v3_v4 (chratos::transaction const & transaction_a)
{
	...
	for (auto i (begin (transaction_a)), n (end ()); i != n; ++i)
	{
		...
		if (!value.key.is_zero ())
		{
			switch (key_type (i->second))
			{
				...
				{
					chratos::raw_key key;
					if (fetch (transaction_a, chratos::public_key (i->first), key))
					{
						// Key failed to decrypt despite valid password
						...
						erase (transaction_a, chratos::public_key (i->first));
						entry_put_raw (transaction_a, chratos::public_key (i->first), new_value);
					}
				}
				case chratos::key_type::deterministic:
					break;
...
chratos::wallet::wallet (bool & init_a, chratos::transaction & transaction_a, chratos::wallets & wallets_a, std::string const & wallet_a) :
...
chratos::wallet::wallet (bool & init_a, chratos::transaction & transaction_a, chratos::wallets & wallets_a, std::string const & wallet_a, std::string const & json) :
lock_observer ([](bool, bool) {}),
...
bool chratos::wallet::enter_password (chratos::transaction const & transaction_a, std::string const & password_a)
{
...
chratos::public_key chratos::wallet::deterministic_insert (chratos::transaction const & transaction_a, bool generate_work_a)
{
...
chratos::public_key chratos::wallet::insert_adhoc (chratos::transaction const & transaction_a, chratos::raw_key const & key_a, bool generate_work_a)
{
	chratos::public_key key (0);
	if (store.valid_password (transaction_a))
	{
...
void chratos::wallet::insert_watch (chratos::transaction const & transaction_a, chratos::public_key const & pub_a)
{
...
bool chratos::wallet::import (std::string const & json_a, std::string const & password_a)
{
	...
	std::unique_ptr<chratos::wallet_store> temp;
	{
		...
		temp.reset (new chratos::wallet_store (error, wallets.node.wallets.kdf, transaction, 0, 1, id.to_string (), json_a));
	}
	if (!error)
	{
		auto transaction (wallets.tx_begin_read ());
...
void chratos::wallet_store::destroy (chratos::transaction const & transaction_a)
{
...
std::shared_ptr<chratos::block> chratos::wallet::receive_action (chratos::block const & send_a, chratos::account const & representative_a, chratos::uint128_union const & amount_a, bool generate_work_a, bool force)
{
	...
	if (wallets.node.config.receive_minimum.number () <= amount_a.number ())
	{
		...
		if (wallets.node.store.block_exists (transaction, hash))
		{
			...
			if (!wallets.node.ledger.store.pending_get (transaction, chratos::pending_key (account, hash), pending_info))
			{
				chratos::raw_key prv;
				if (!store.fetch (transaction, account, prv))
				{
					...
					if (!new_account)
					{
						...
						if (dividend == info.dividend_block || wallets.node.ledger.dividends_are_ordered (transaction, dividend, info.dividend_block))
						{
							std::shared_ptr<chratos::block> rep_block = wallets.node.ledger.store.block_get (transaction, info.rep_block);
						...
						{
							BOOST_LOG (wallets.node.log) << "Unable to receive, incorrect dividend hash";
						}
					}
					else
					{
			...
			if (store.valid_password (transaction))
			{
				...
				if (existing != store.end ())
				{
					...
					if (!balance.is_zero () && balance >= amount_a)
					{
						...
						assert (!error2);
						std::shared_ptr<chratos::block> rep_block = wallets.node.ledger.store.block_get (transaction, info.rep_block);
						chratos::dividend_info div_info (wallets.node.ledger.store.dividend_get (transaction));
						assert (rep_block != nullptr);
						uint64_t cached_work (0);
						store.work_get (transaction, source_a, cached_work);
						...
						{
							auto status (mdb_put (wallets.env.tx (transaction), wallets.node.wallets.send_action_ids, *id_mdb_val, chratos::mdb_val (block->hash ()), 0));
							if (status != 0)
							{
								block = nullptr;
								error = true;
...
std::shared_ptr<chratos::block> chratos::wallet::pay_dividend_action (chratos::account const & source_a, chratos::uint128_t const & amount_a, bool generate_work_a, boost::optional<std::string> id_a)
{
	...
	bool cached_block = false;
	{
		...
		{
			chratos::mdb_val result;
			auto status (mdb_get (wallets.env.tx (transaction), wallets.pay_dividend_action_ids, *id_mdb_val, result));
			if (status == 0)
			{
				chratos::uint256_union hash (result);
				block = wallets.node.store.block_get (transaction, hash);
			...
			if (store.valid_password (transaction))
			{
				...
				if (existing != store.end ())
				{
					...
					if (!balance.is_zero () && balance >= amount_a)
					{
						...
						if (id_mdb_val && block != nullptr)
						{
							auto status (mdb_put (wallets.env.tx (transaction), wallets.pay_dividend_action_ids, *id_mdb_val, chratos::mdb_val (block->hash ()), 0));
							if (status != 0)
							{
								block = nullptr;
								error = true;
...
std::shared_ptr<chratos::block> chratos::wallet::claim_dividend_action (chratos::block const & dividend_a, chratos::account const & account_a, chratos::account const & representative_a, bool generate_work_a)
{
	...
	if (wallets.node.store.block_exists (transaction, hash))
	{
		dividend_block = wallets.node.store.block_get (transaction, hash);
		if (!has_outstanding_pendings_for_dividend (transaction, dividend_block, account_a))
		{
			if (!wallets.node.ledger.store.account_get (transaction, account_a, account_info))
			{
				if (dividend_block->dividend () == account_info.dividend_block)
				{
					...
					if (!store.fetch (transaction, account_a, prv))
					{
						...
						std::shared_ptr<chratos::block> rep_block = wallets.node.ledger.store.block_get (transaction, account_info.rep_block);
						assert (rep_block != nullptr);
...
{
	wallets.node.wallets.queue_wallet_action (chratos::wallets::high_priority, shared_from_this (), [source_a, representative_a, action_a, generate_work_a](chratos::wallet & wallet_a) {
...
void chratos::wallet::receive_async (std::shared_ptr<chratos::block> block_a, chratos::account const & representative_a, chratos::uint128_t const & amount_a, std::function<void(std::shared_ptr<chratos::block>)> const & action_a, bool generate_work_a, bool force_a)
{
	...
	wallets.node.wallets.queue_wallet_action (amount_a, shared_from_this (), [block_a, representative_a, amount_a, action_a, generate_work_a](chratos::wallet & wallet_a) {
		auto block (wallet_a.receive_action (*static_cast<chratos::block *> (block_a.get ()), representative_a, amount_a, generate_work_a));
		action_a (block);
	});
}
...
{
	wallets.node.wallets.queue_wallet_action (chratos::wallets::high_priority, shared_from_this (), [source_a, account_a, amount_a, action_a, generate_work_a, id_a](chratos::wallet & wallet_a) {
...
{
	wallets.queue_wallet_action (chratos::wallets::high_priority, shared_from_this (), [source_a, amount_a, action_a, generate_work_a, id_a](chratos::wallet & wallet_a) {
		auto block (wallet_a.pay_dividend_action (source_a, amount_a, generate_work_a, id_a));
		action_a (block);
	});
}
...
{
	wallets.queue_wallet_action (chratos::wallets::high_priority, shared_from_this (), [dividend_a, account_a, representative_a, action_a, generate_work_a](chratos::wallet & wallet_a) {
		auto block (wallet_a.claim_dividend_action (*static_cast<chratos::block *> (dividend_a.get ()), account_a, representative_a, generate_work_a));
		action_a (block);
	});
}

// Update work for account if latest root is root_a
void chratos::wallet::work_update (chratos::transaction const & transaction_a, chratos::account const & account_a, chratos::block_hash const & root_a, uint64_t work_a)
{
...
{
	wallets.node.wallets.queue_wallet_action (chratos::wallets::generate_priority, shared_from_this (), [account_a, hash_a](chratos::wallet & wallet_a) {
...
bool chratos::wallet::search_pending ()
{
	...
	if (!result)
	{
		...
		for (auto i (store.begin (transaction)), n (store.end ()); i != n; ++i)
		{
			...
			{
				for (auto j (wallets.node.store.pending_begin (transaction, chratos::pending_key (account, 0))); chratos::pending_key (j->first).account == account; ++j)
				{
...
std::vector<chratos::dividend_claim_result> chratos::wallet::claim_dividends ()
{
	...
	auto transaction (wallets.tx_begin_read ());
	auto dividend_order (wallets.node.ledger.get_dividend_indexes (transaction));
	...
	for (auto & hash : ordered)
	{
		...
		std::shared_ptr<chratos::block> dividend_l (wallets.node.store.block_get (transaction, hash));
		for (auto & account : accounts)
		{
			...
			chratos::account_info info;
			wallets.node.store.account_get (transaction, account, info);
			// Claim dividends
...
{
	auto transaction (wallets.tx_begin_read ());
	return wallets.node.ledger.unclaimed_for_account (transaction, account_a);
}
...
std::vector<chratos::account> chratos::wallet::search_unclaimed (chratos::block_hash const & dividend_a)
{
	...
	if (!error)
	{
		BOOST_LOG (wallets.node.log) << "Beginning unclaimed dividend search for " << dividend_a.to_string ();
		for (auto i (store.begin (transaction)), n (store.end ()); i != n; ++i)
		{
			chratos::account account (i->first);
			if (!chratos::wallet_value (i->second).key.is_zero ())
			{
				auto div_info (wallets.node.store.dividend_get (transaction));
				chratos::account_info info;

				if (wallets.node.store.account_get (transaction, account, info))
				{
					break;
				}
				...
				while (current != chratos::uint256_t (0) && claimed)
				{
					...
					auto block = wallets.node.store.block_get (transaction, current);
...
chratos::uint128_union chratos::wallet::amount_for_dividend (chratos::transaction const & transaction_a, std::shared_ptr<chratos::block> block_a, chratos::account const & account_a)
{
	return wallets.node.ledger.amount_for_dividend (transaction_a, block_a->hash (), account_a);
}

bool chratos::wallet::has_outstanding_pendings_for_dividend (chratos::transaction const & transaction_a, std::shared_ptr<chratos::block> block_a, chratos::account const & account_a)
{
	bool result (false);

	chratos::account end (account_a.number () + 1);
	for (auto i (wallets.node.store.pending_begin (transaction_a, chratos::pending_key (account_a, 0))), n (wallets.node.store.pending_begin (transaction_a, chratos::pending_key (end, 0))); i != n && !result; ++i)
	{
...
void chratos::wallet::receive_outstanding_pendings_sync (chratos::transaction const & transaction_a, chratos::account const & account_a, chratos::block_hash const & dividend_a)
{
	const auto div_block = wallets.node.ledger.store.block_get (transaction_a, dividend_a);
  ...
  representative = store.representative (transaction_a);
	...
	for (auto j (wallets.node.store.pending_begin (transaction_a, chratos::pending_key (account_a, 0))), m (wallets.node.store.pending_begin (transaction_a, chratos::pending_key (account_a.number () + 1, 0))); j != m; ++j)
	{
		chratos::pending_key key (j->first);
		chratos::pending_info pending (j->second);
		if (pending.dividend == last_dividend_hash)
		{
			auto hash (key.hash);
			auto amount (pending.amount.number ());
			std::shared_ptr<chratos::block> block = wallets.node.store.block_get (transaction_a, hash);
			receive_sync (block, representative, amount, true);
		}
	}
}

void chratos::wallet::receive_outstanding_pendings_async (chratos::transaction const & transaction_a, chratos::account const & account_a, chratos::block_hash const & dividend_a, std::function<void()> const & action_a, bool generate_work_a)
{
  const auto div_block = wallets.node.ledger.store.block_get (transaction_a, dividend_a);
  const auto last_dividend_hash = div_block->dividend ();
  ...
  auto pending (wallets.node.store.pending_begin (transaction_a, chratos::pending_key (account_a, 0)));

  auto end (wallets.node.store.pending_begin (transaction_a, chratos::pending_key (account_a.number () + 1, 0)));
...
void chratos::wallet::init_free_accounts (chratos::transaction const & transaction_a)
{
	free_accounts.clear ();
...
chratos::public_key chratos::wallet::change_seed (chratos::transaction const & transaction_a, chratos::raw_key const & prv_a)
{
	...
	for (uint32_t i (1), n (64); i < n; ++i)
	{
		...
		else
		{
			...
			for (auto ii (wallets.node.store.pending_begin (transaction_a, chratos::pending_key (pair.pub, 0))); chratos::pending_key (ii->first).account == pair.pub; ++ii)
			{
				count = i;
				n = i + 64 + (i / 64);
				break;
...
observer ([](bool) {}),
node (node_a),
env (boost::polymorphic_downcast<chratos::mdb_store *> (node_a.store_impl.get ())->env),
stopped (false),
thread ([this]() {
	chratos::thread_role::set (chratos::thread_role::name::wallet_actions);
	do_wallet_actions ();
})
{
	if (!error_a)
	{
		...
		status |= mdb_dbi_open (env.tx (transaction), "pay_dividend_action_ids", MDB_CREATE, &pay_dividend_action_ids);
		assert (status == 0);
		...
		for (; i != n; ++i)
		{
			...
			auto wallet (std::make_shared<chratos::wallet> (error, transaction, *this, text));
			if (!error)
			{
				items[id] = wallet;
			}
...
std::shared_ptr<chratos::wallet> chratos::wallets::create (chratos::uint256_union const & id_a)
{
	...
	{
		auto transaction (node.store.tx_begin_write ());
		result = std::make_shared<chratos::wallet> (error, transaction, *this, id_a.to_string ());
	}
	if (!error)
	{
		items[id_a] = result;
...
void chratos::wallets::queue_wallet_action (chratos::uint128_t const & amount_a, std::shared_ptr<chratos::wallet> wallet_a, std::function<void(chratos::wallet &)> const & action_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void chratos::wallets::foreach_representative (chratos::transaction const & transaction_a, std::function<void(chratos::public_key const & pub_a, chratos::raw_key const & prv_a)> const & action_a)
{
	for (auto i (items.begin ()), n (items.end ()); i != n; ++i)
	{
...
bool chratos::wallets::exists (chratos::transaction const & transaction_a, chratos::public_key const & account_a)
{
	auto result (false);
...
chratos::transaction chratos::wallets::tx_begin_write ()
{
	return tx_begin (true);
}

chratos::transaction chratos::wallets::tx_begin_read ()
{
	return tx_begin (false);
}

chratos::transaction chratos::wallets::tx_begin (bool write_a)
{
	return env.tx_begin (write_a);
}

void chratos::wallets::clear_send_ids (chratos::transaction const & transaction_a)
{
...
chratos::store_iterator<chratos::uint256_union, chratos::wallet_value> chratos::wallet_store::begin (chratos::transaction const & transaction_a)
{
...
chratos::store_iterator<chratos::uint256_union, chratos::wallet_value> chratos::wallet_store::begin (chratos::transaction const & transaction_a, chratos::uint256_union const & key)
{
...
chratos::store_iterator<chratos::uint256_union, chratos::wallet_value> chratos::wallet_store::find (chratos::transaction const & transaction_a, chratos::uint256_union const & key)
{
...
MDB_txn * chratos::wallet_store::tx (chratos::transaction const & transaction_a) const
{
