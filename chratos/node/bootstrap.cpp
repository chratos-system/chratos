...
chratos::socket::socket (std::shared_ptr<chratos::node> node_a) :
...
void chratos::socket::async_connect (chratos::tcp_endpoint const & endpoint_a, std::function<void(boost::system::error_code const &)> callback_a)
{
...
void chratos::socket::async_read (std::shared_ptr<std::vector<uint8_t>> buffer_a, size_t size_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
...
void chratos::socket::async_write (std::shared_ptr<std::vector<uint8_t>> buffer_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
	auto this_l (shared_from_this ());
	start ();
...
void chratos::socket::start (std::chrono::steady_clock::time_point timeout_a)
{
	auto ticket_l (++ticket);
	std::weak_ptr<chratos::socket> this_w (shared_from_this ());
...
void chratos::socket::stop ()
{
	++ticket;
}

void chratos::socket::close ()
{
	if (socket_m.is_open ())
	{
		socket_m.close ();
...
chratos::tcp_endpoint chratos::socket::remote_endpoint ()
{
...
chratos::bootstrap_client::bootstrap_client (std::shared_ptr<chratos::node> node_a, std::shared_ptr<chratos::bootstrap_attempt> attempt_a, chratos::tcp_endpoint const & endpoint_a) :
node (node_a),
attempt (attempt_a),
socket (std::make_shared<chratos::socket> (node_a)),
...
chratos::bootstrap_client::~bootstrap_client ()
{
	--attempt->connections;
}

double chratos::bootstrap_client::block_rate () const
{
	auto elapsed = elapsed_seconds ();
	return elapsed > 0.0 ? (double)block_count.load () / elapsed : 0.0;
}

double chratos::bootstrap_client::elapsed_seconds () const
{
	return std::chrono::duration_cast<std::chrono::duration<double>> (std::chrono::steady_clock::now () - start_time).count ();
}

void chratos::bootstrap_client::stop (bool force)
{
	pending_stop = true;
	if (force)
	{
...
void chratos::bootstrap_client::run ()
{
	auto this_l (shared_from_this ());
...
void chratos::frontier_req_client::run ()
{
	std::unique_ptr<chratos::frontier_req> request (new chratos::frontier_req);
	request->start.clear ();
...
std::shared_ptr<chratos::bootstrap_client> chratos::bootstrap_client::shared ()
{
	return shared_from_this ();
}

chratos::frontier_req_client::frontier_req_client (std::shared_ptr<chratos::bootstrap_client> connection_a) :
...
chratos::frontier_req_client::~frontier_req_client ()
{
}

void chratos::frontier_req_client::receive_frontier ()
{
	auto this_l (shared_from_this ());
	size_t size_l (sizeof (chratos::uint256_union) + sizeof (chratos::uint256_union));
...
void chratos::frontier_req_client::unsynced (chratos::block_hash const & head, chratos::block_hash const & end)
{
...
void chratos::frontier_req_client::received_frontier (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == sizeof (chratos::uint256_union) + sizeof (chratos::uint256_union));
		chratos::account account;
		chratos::bufferstream account_stream (connection->receive_buffer->data (), sizeof (chratos::uint256_union));
		auto error1 (chratos::read (account_stream, account));
		assert (!error1);
		chratos::block_hash latest;
		chratos::bufferstream latest_stream (connection->receive_buffer->data () + sizeof (chratos::uint256_union), sizeof (chratos::uint256_union));
		auto error2 (chratos::read (latest_stream, latest));
		assert (!error2);
		if (count == 0)
		{
			start_time = std::chrono::steady_clock::now ();
			...
			if (!current.is_zero ())
			{
				if (account == current)
				{
					...
					else
					{
						...
						{
							connection->attempt->add_pull (chratos::pull_info (account, latest, info.head));
				...
				else
				{
					...
					connection->attempt->add_pull (chratos::pull_info (account, latest, chratos::block_hash (0)));
				}
			}
			else
			{
...
void chratos::frontier_req_client::next (MDB_txn * transaction_a)
{
	...
	{
		current = chratos::account (iterator->first);
		info = chratos::account_info (iterator->second);
	}
	else
	{
		current.clear ();
...
chratos::bulk_pull_client::bulk_pull_client (std::shared_ptr<chratos::bootstrap_client> connection_a, chratos::pull_info const & pull_a) :
connection (connection_a),
pull (pull_a)
{
...
chratos::bulk_pull_client::~bulk_pull_client ()
{
...
void chratos::bulk_pull_client::request ()
{
	expected = pull.head;
	chratos::bulk_pull req;
...
void chratos::bulk_pull_client::receive_block ()
{
	auto this_l (shared_from_this ());
...
void chratos::bulk_pull_client::received_type ()
{
	auto this_l (shared_from_this ());
	chratos::block_type type (static_cast<chratos::block_type> (connection->receive_buffer->data ()[0]));
	switch (type)
	{
		case chratos::block_type::send:
		{
			connection->socket->async_read (connection->receive_buffer, chratos::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
		...
		{
			connection->socket->async_read (connection->receive_buffer, chratos::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, chratos::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, chratos::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void chratos::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a, chratos::block_type type_a)
{
	if (!ec)
	{
		chratos::bufferstream stream (connection->receive_buffer->data (), size_a);
		std::shared_ptr<chratos::block> block (chratos::deserialize_block (stream, type_a));
		if (block != nullptr && !chratos::work_validate (*block))
		{
...
chratos::bulk_push_client::bulk_push_client (std::shared_ptr<chratos::bootstrap_client> const & connection_a) :
connection (connection_a)
{
}

chratos::bulk_push_client::~bulk_push_client ()
{
}

void chratos::bulk_push_client::start ()
{
	chratos::bulk_push message;
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		chratos::vectorstream stream (*buffer);
...
void chratos::bulk_push_client::push (MDB_txn * transaction_a)
{
	...
	while (block == nullptr && !finished)
	{
		...
		if (!finished)
		{
			...
			if (block == nullptr)
			{
				current_target.first = chratos::block_hash (0);
			}
			else
			{
				if (connection->node->config.logging.bulk_pull_logging ())
...
void chratos::bulk_push_client::send_finished ()
{
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	buffer->push_back (static_cast<uint8_t> (chratos::block_type::not_a_block));
	connection->node->stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::bulk_push, chratos::stat::dir::out);
	if (connection->node->config.logging.network_logging ())
	{
...
void chratos::bulk_push_client::push_block (chratos::block const & block_a)
{
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		chratos::vectorstream stream (*buffer);
...
chratos::pull_info::pull_info (chratos::account const & account_a, chratos::block_hash const & head_a, chratos::block_hash const & end_a) :
account (account_a),
head (head_a),
end (end_a),
attempts (0)
...
chratos::bootstrap_attempt::bootstrap_attempt (std::shared_ptr<chratos::node> node_a) :
...
chratos::bootstrap_attempt::~bootstrap_attempt ()
{
	BOOST_LOG (node->log) << "Exiting bootstrap attempt";
	node->bootstrap_initiator.notify_listeners (false);
}

bool chratos::bootstrap_attempt::should_log ()
{
...
bool chratos::bootstrap_attempt::request_frontier (std::unique_lock<std::mutex> & lock_a)
{
	auto result (true);
	...
	if (connection_l)
	{
		...
		{
			auto client (std::make_shared<chratos::frontier_req_client> (connection_l));
			client->run ();
			frontiers = client;
...
void chratos::bootstrap_attempt::request_pull (std::unique_lock<std::mutex> & lock_a)
{
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		...
		node->background ([connection_l, pull]() {
			auto client (std::make_shared<chratos::bulk_pull_client> (connection_l, pull));
			client->request ();
		});
	}
}

void chratos::bootstrap_attempt::request_push (std::unique_lock<std::mutex> & lock_a)
{
	bool error (false);
	if (auto connection_shared = connection_frontier_request.lock ())
	{
		auto client (std::make_shared<chratos::bulk_push_client> (connection_shared));
		client->start ();
		push = client;
...
bool chratos::bootstrap_attempt::still_pulling ()
{
...
void chratos::bootstrap_attempt::run ()
{
	...
	{
		auto k = chratos::random_pool.GenerateWord32 (0, i);
...
std::shared_ptr<chratos::bootstrap_client> chratos::bootstrap_attempt::connection (std::unique_lock<std::mutex> & lock_a)
{
	while (!stopped && idle.empty ())
	{
		condition.wait (lock_a);
	}
	std::shared_ptr<chratos::bootstrap_client> result;
	if (!idle.empty ())
	{
		result = idle.back ();
		idle.pop_back ();
...
bool chratos::bootstrap_attempt::consume_future (std::future<bool> & future_a)
{
	bool result;
	try
	{
...
{
	bool operator() (const std::shared_ptr<chratos::bootstrap_client> & lhs, const std::shared_ptr<chratos::bootstrap_client> & rhs) const
	{
		return lhs->block_rate () > rhs->block_rate ();
	}
};

unsigned chratos::bootstrap_attempt::target_connections (size_t pulls_remaining)
{
...
void chratos::bootstrap_attempt::populate_connections ()
{
	double rate_sum = 0.0;
	size_t num_pulls = 0;
	std::priority_queue<std::shared_ptr<chratos::bootstrap_client>, std::vector<std::shared_ptr<chratos::bootstrap_client>>, block_rate_cmp> sorted_connections;
	{
		std::unique_lock<std::mutex> lock (mutex);
	...
	if (connections < target)
	{
		...
		for (int i = 0; i < delta; i++)
		{
			...
			if (peer != chratos::endpoint (boost::asio::ip::address_v6::any (), 0))
			{
				auto client (std::make_shared<chratos::bootstrap_client> (node, shared_from_this (), chratos::tcp_endpoint (peer.address (), peer.port ())));
				client->run ();
				std::lock_guard<std::mutex> lock (mutex);
	...
	if (!stopped)
	{
		std::weak_ptr<chratos::bootstrap_attempt> this_w (shared_from_this ());
...
void chratos::bootstrap_attempt::add_connection (chratos::endpoint const & endpoint_a)
{
	auto client (std::make_shared<chratos::bootstrap_client> (node, shared_from_this (), chratos::tcp_endpoint (endpoint_a.address (), endpoint_a.port ())));
	client->run ();
}

void chratos::bootstrap_attempt::pool_connection (std::shared_ptr<chratos::bootstrap_client> client_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void chratos::bootstrap_attempt::stop ()
{
	std::lock_guard<std::mutex> lock (mutex);
	stopped = true;
	condition.notify_all ();
...
void chratos::bootstrap_attempt::add_pull (chratos::pull_info const & pull)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void chratos::bootstrap_attempt::requeue_pull (chratos::pull_info const & pull_a)
{
	auto pull (pull_a);
	...
	else if (pull.attempts == bootstrap_frontier_retry_limit)
	{
		...
		if (auto connection_shared = connection_frontier_request.lock ())
		{
			node->background ([connection_shared, pull]() {
				auto client (std::make_shared<chratos::bulk_pull_client> (connection_shared, pull));
				client->request ();
			});
			if (node->config.logging.bulk_pull_logging ())
			{
...
void chratos::bootstrap_attempt::add_bulk_push_target (chratos::block_hash const & head, chratos::block_hash const & end)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void chratos::bootstrap_initiator::bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped && attempt == nullptr)
	{
		node.stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::initiate, chratos::stat::dir::out);
		attempt = std::make_shared<chratos::bootstrap_attempt> (node.shared ());
		condition.notify_all ();
	}
}

void chratos::bootstrap_initiator::bootstrap (chratos::endpoint const & endpoint_a, bool add_to_peers)
{
	if (add_to_peers)
	{
		node.peers.insert (chratos::map_endpoint_to_v6 (endpoint_a), chratos::protocol_version);
	}
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
...
void chratos::bootstrap_initiator::run_bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	while (!stopped)
	{
...
void chratos::bootstrap_initiator::add_observer (std::function<void(bool)> const & observer_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	observers.push_back (observer_a);
}

bool chratos::bootstrap_initiator::in_progress ()
{
	return current_attempt () != nullptr;
}

std::shared_ptr<chratos::bootstrap_attempt> chratos::bootstrap_initiator::current_attempt ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return attempt;
}

void chratos::bootstrap_initiator::stop ()
{
	std::unique_lock<std::mutex> lock (mutex);
	stopped = true;
	if (attempt != nullptr)
...
void chratos::bootstrap_initiator::notify_listeners (bool in_progress_a)
{
...
chratos::bootstrap_listener::bootstrap_listener (boost::asio::io_service & service_a, uint16_t port_a, chratos::node & node_a) :
acceptor (service_a),
...
void chratos::bootstrap_listener::start ()
{
...
void chratos::bootstrap_listener::stop ()
{
...
void chratos::bootstrap_listener::accept_connection ()
{
	auto socket (std::make_shared<chratos::socket> (node.shared ()));
	acceptor.async_accept (socket->socket_m, [this, socket](boost::system::error_code const & ec) {
		accept_action (ec, socket);
	});
}

void chratos::bootstrap_listener::accept_action (boost::system::error_code const & ec, std::shared_ptr<chratos::socket> socket_a)
{
	if (!ec)
	{
		accept_connection ();
		auto connection (std::make_shared<chratos::bootstrap_server> (socket_a, node.shared ()));
		{
			std::lock_guard<std::mutex> lock (mutex);
...
boost::asio::ip::tcp::endpoint chratos::bootstrap_listener::endpoint ()
{
	return boost::asio::ip::tcp::endpoint (boost::asio::ip::address_v6::loopback (), local.port ());
}

chratos::bootstrap_server::~bootstrap_server ()
{
	if (node->config.logging.bulk_pull_logging ())
	{
...
chratos::bootstrap_server::bootstrap_server (std::shared_ptr<chratos::socket> socket_a, std::shared_ptr<chratos::node> node_a) :
receive_buffer (std::make_shared<std::vector<uint8_t>> ()),
socket (socket_a),
node (node_a)
{
...
void chratos::bootstrap_server::receive ()
{
	auto this_l (shared_from_this ());
...
void chratos::bootstrap_server::receive_header_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 8);
		chratos::bufferstream type_stream (receive_buffer->data (), size_a);
		auto error (false);
		chratos::message_header header (error, type_stream);
		if (!error)
		{
			switch (header.type)
			{
				case chratos::message_type::bulk_pull:
				{
					node->stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::bulk_pull, chratos::stat::dir::in);
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, sizeof (chratos::uint256_union) + sizeof (chratos::uint256_union), [this_l, header](boost::system::error_code const & ec, size_t size_a) {
						this_l->receive_bulk_pull_action (ec, size_a, header);
					});
					break;
				}
				case chratos::message_type::bulk_pull_account:
				{
					node->stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::bulk_pull_account, chratos::stat::dir::in);
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, sizeof (chratos::uint256_union) + sizeof (chratos::uint128_union) + sizeof (uint8_t), [this_l, header](boost::system::error_code const & ec, size_t size_a) {
				...
				case chratos::message_type::frontier_req:
				{
					node->stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::frontier_req, chratos::stat::dir::in);
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, sizeof (chratos::uint256_union) + sizeof (uint32_t) + sizeof (uint32_t), [this_l, header](boost::system::error_code const & ec, size_t size_a) {
						this_l->receive_frontier_req_action (ec, size_a, header);
					});
					break;
				}
				case chratos::message_type::bulk_push:
				{
					node->stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::bulk_push, chratos::stat::dir::in);
					add_request (std::unique_ptr<chratos::message> (new chratos::bulk_push (header)));
					break;
				}
				default:
				{
...
void chratos::bootstrap_server::receive_bulk_pull_action (boost::system::error_code const & ec, size_t size_a, chratos::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		chratos::bufferstream stream (receive_buffer->data (), sizeof (chratos::uint256_union) + sizeof (chratos::uint256_union));
		std::unique_ptr<chratos::bulk_pull> request (new chratos::bulk_pull (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
...
void chratos::bootstrap_server::receive_bulk_pull_account_action (boost::system::error_code const & ec, size_t size_a, chratos::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		assert (size_a == (sizeof (chratos::uint256_union) + sizeof (chratos::uint128_union) + sizeof (uint8_t)));
		chratos::bufferstream stream (receive_buffer->data (), size_a);
		std::unique_ptr<chratos::bulk_pull_account> request (new chratos::bulk_pull_account (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
				BOOST_LOG (node->log) << boost::str (boost::format ("Received bulk pull account for %1% with a minimum amount of %2%") % request->account.to_account () % chratos::amount (request->minimum_amount).format_balance (chratos::Mchr_ratio, 10, true));
			}
			add_request (std::unique_ptr<chratos::message> (request.release ()));
			receive ();
		}
...
void chratos::bootstrap_server::receive_bulk_pull_blocks_action (boost::system::error_code const & ec, size_t size_a, chratos::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		chratos::bufferstream stream (receive_buffer->data (), sizeof (chratos::uint256_union) + sizeof (chratos::uint256_union) + sizeof (bulk_pull_blocks_mode) + sizeof (uint32_t));
		std::unique_ptr<chratos::bulk_pull_blocks> request (new chratos::bulk_pull_blocks (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
...
void chratos::bootstrap_server::receive_frontier_req_action (boost::system::error_code const & ec, size_t size_a, chratos::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		chratos::bufferstream stream (receive_buffer->data (), sizeof (chratos::uint256_union) + sizeof (uint32_t) + sizeof (uint32_t));
		std::unique_ptr<chratos::frontier_req> request (new chratos::frontier_req (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
...
void chratos::bootstrap_server::add_request (std::unique_ptr<chratos::message> message_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void chratos::bootstrap_server::finish_request ()
{
	std::lock_guard<std::mutex> lock (mutex);
	requests.pop ();
	if (!requests.empty ())
...
namespace
{
class request_response_visitor : public chratos::message_visitor
{
public:
	request_response_visitor (std::shared_ptr<chratos::bootstrap_server> connection_a) :
	connection (connection_a)
	{
	}
	...
	{
		auto response (std::make_shared<chratos::bulk_pull_server> (connection, std::unique_ptr<chratos::bulk_pull> (static_cast<chratos::bulk_pull *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
	void bulk_pull_account (chratos::bulk_pull_account const &) override
	{
		auto response (std::make_shared<chratos::bulk_pull_account_server> (connection, std::unique_ptr<chratos::bulk_pull_account> (static_cast<chratos::bulk_pull_account *> (connection->requests.front ().release ()))));
		response->send_frontier ();
	}
	void bulk_pull_blocks (chratos::bulk_pull_blocks const &) override
	{
		auto response (std::make_shared<chratos::bulk_pull_blocks_server> (connection, std::unique_ptr<chratos::bulk_pull_blocks> (static_cast<chratos::bulk_pull_blocks *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
	void bulk_push (chratos::bulk_push const &) override
	{
		auto response (std::make_shared<chratos::bulk_push_server> (connection));
		response->receive ();
	}
	void frontier_req (chratos::frontier_req const &) override
	{
		auto response (std::make_shared<chratos::frontier_req_server> (connection, std::unique_ptr<chratos::frontier_req> (static_cast<chratos::frontier_req *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
...
void chratos::bootstrap_server::run_next ()
{
 ...
 */
void chratos::bulk_pull_server::set_current_end ()
{
	include_start = false;
...
void chratos::bulk_pull_server::send_next ()
{
...
std::unique_ptr<chratos::block> chratos::bulk_pull_server::get_next ()
{
...
void chratos::bulk_pull_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void chratos::bulk_pull_server::send_finished ()
{
	send_buffer->clear ();
...
void chratos::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
chratos::bulk_pull_server::bulk_pull_server (std::shared_ptr<chratos::bootstrap_server> const & connection_a, std::unique_ptr<chratos::bulk_pull> request_a) :
connection (connection_a),
 ...
 */
void chratos::bulk_pull_account_server::set_params ()
{
	assert (request != nullptr);
...
void chratos::bulk_pull_account_server::send_frontier ()
{
	/*
	...
	chratos::uint128_union account_frontier_balance (account_frontier_balance_int);
	...
	send_buffer->clear ();
	{
		chratos::vectorstream output_stream (*send_buffer);
...
void chratos::bulk_pull_account_server::send_next_block ()
{
	/*
...
std::pair<std::unique_ptr<chratos::pending_key>, std::unique_ptr<chratos::pending_info>> chratos::bulk_pull_account_server::get_next ()
{
	std::pair<std::unique_ptr<chratos::pending_key>, std::unique_ptr<chratos::pending_info>> result;
	...
	while (true)
	{
		...
		chratos::pending_key key (stream->first);
		chratos::pending_info info (stream->second);
		...
		result.first = std::unique_ptr<chratos::pending_key> (new chratos::pending_key (key));
		result.second = std::unique_ptr<chratos::pending_info> (new chratos::pending_info (info));
...
void chratos::bulk_pull_account_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next_block ();
...
void chratos::bulk_pull_account_server::send_finished ()
{
	/*
	...
	{
		chratos::vectorstream output_stream (*send_buffer);
		chratos::uint256_union account_zero (0);
		chratos::uint128_union balance_zero (0);
...
void chratos::bulk_pull_account_server::complete (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		if (pending_address_only)
...
chratos::bulk_pull_account_server::bulk_pull_account_server (std::shared_ptr<chratos::bootstrap_server> const & connection_a, std::unique_ptr<chratos::bulk_pull_account> request_a) :
connection (connection_a),
request (std::move (request_a)),
send_buffer (std::make_shared<std::vector<uint8_t>> ()),
current_key (0, 0)
...
void chratos::bulk_pull_blocks_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
chratos::bulk_push_server::bulk_push_server (std::shared_ptr<chratos::bootstrap_server> const & connection_a) :
receive_buffer (std::make_shared<std::vector<uint8_t>> ()),
connection (connection_a)
{
	receive_buffer->resize (256);
...
void chratos::bulk_push_server::receive ()
{
	auto this_l (shared_from_this ());
...
void chratos::bulk_push_server::received_type ()
{
	auto this_l (shared_from_this ());
	chratos::block_type type (static_cast<chratos::block_type> (receive_buffer->data ()[0]));
	switch (type)
	{
		case chratos::block_type::send:
		{
			connection->node->stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::send, chratos::stat::dir::in);
			connection->socket->async_read (receive_buffer, chratos::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case chratos::block_type::receive:
		{
			connection->node->stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::receive, chratos::stat::dir::in);
			connection->socket->async_read (receive_buffer, chratos::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case chratos::block_type::open:
		{
			connection->node->stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::open, chratos::stat::dir::in);
			connection->socket->async_read (receive_buffer, chratos::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case chratos::block_type::change:
		{
			connection->node->stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::change, chratos::stat::dir::in);
			connection->socket->async_read (receive_buffer, chratos::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case chratos::block_type::state:
		{
			connection->node->stats.inc (chratos::stat::type::bootstrap, chratos::stat::detail::state_block, chratos::stat::dir::in);
			connection->socket->async_read (receive_buffer, chratos::state_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void chratos::bulk_push_server::received_block (boost::system::error_code const & ec, size_t size_a, chratos::block_type type_a)
{
	if (!ec)
	{
		chratos::bufferstream stream (receive_buffer->data (), size_a);
		auto block (chratos::deserialize_block (stream, type_a));
		if (block != nullptr && !chratos::work_validate (*block))
		{
...
void chratos::frontier_req_server::skip_old ()
{
...
void chratos::frontier_req_server::send_next ()
{
	if (!current.is_zero ())
	{
		{
...
void chratos::frontier_req_server::send_finished ()
{
	{
		send_buffer->clear ();
		chratos::vectorstream stream (*send_buffer);
...
void chratos::frontier_req_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		connection->finish_request ();
...
void chratos::frontier_req_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void chratos::frontier_req_server::next ()
{
	...
	if (iterator != connection->node->store.latest_end ())
	{
		current = chratos::uint256_union (iterator->first);
		info = chratos::account_info (iterator->second);
	}
	else
	{
