...
public:
	...
	chratos::observer_set<boost::circular_buffer<stat_datapoint> &> sample_observers;

	/** Observers for count. Called on each update. */
	chratos::observer_set<uint64_t, uint64_t> count_observers;
};
...
public:
	 ...
	 */
	stat (chratos::stat_config config);
...
private:
	...
	std::shared_ptr<chratos::stat_entry> get_entry (uint32_t key);

	/** Get entry for key, creating a new entry if necessary */
	std::shared_ptr<chratos::stat_entry> get_entry (uint32_t key, size_t sample_interval, size_t max_samples);

	/** Unlocked implementation of get_entry() */
	std::shared_ptr<chratos::stat_entry> get_entry_impl (uint32_t key, size_t sample_interval, size_t max_samples);
