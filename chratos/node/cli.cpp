...
std::string chratos::error_cli_messages::message (int ev) const
{
	switch (static_cast<chratos::error_cli> (ev))
	{
		case chratos::error_cli::generic:
			return "Unknown error";
		case chratos::error_cli::parse_error:
			return "Coud not parse command line";
		case chratos::error_cli::invalid_arguments:
			return "Invalid arguments";
		case chratos::error_cli::unknown_command:
...
void chratos::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
std::error_code chratos::handle_node_options (boost::program_options::variables_map & vm)
{
	std::error_code ec;
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					else
					{
						...
						ec = chratos::error_cli::invalid_arguments;
					}
				}
				else
				{
	...
	else if (vm.count ("vacuum") > 0)
	{
		try
		{
			...
			bool success = false;
			{
				...
				{
					chratos::transaction transaction (node.node->store.environment, nullptr, true);
	...
	else if (vm.count ("diagnostics"))
	{
		...
		chratos::raw_key key;
		key.data.clear ();
		chratos::state_block send (0, 0, 0, 0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		chratos::raw_key junk1;
		junk1.data.clear ();
		chratos::uint256_union junk2 (0);
		chratos::kdf kdf;
		kdf.phs (junk1, "", junk2);
		...
		if (vm.count ("wallet") == 1)
		{
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (existing != node.node->wallets.items.end ())
				{
					...
					{
						chratos::transaction transaction (existing->second->store.environment, nullptr, false);
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node.node->wallets.items.begin ()), n (node.node->wallets.items.end ()); i != n; ++i)
		{
			...
			chratos::transaction transaction (i->second->store.environment, nullptr, false);
			for (auto j (i->second->store.begin (transaction)), m (i->second->store.end ()); j != m; ++j)
			{
				std::cout << chratos::uint256_union (j->first).to_account () << '\n';
			}
		}
	}
	else if (vm.count ("wallet_remove"))
	{
		if (vm.count ("wallet") == 1 && vm.count ("account") == 1)
		{
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				{
					chratos::account account_id;
					if (!account_id.decode_account (vm["account"].as<std::string> ()))
					{
						chratos::transaction transaction (wallet->second->store.environment, nullptr, true);
	...
	else if (vm.count ("wallet_representative_get"))
	{
		if (vm.count ("wallet") == 1)
		{
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != node.node->wallets.items.end ())
				{
					chratos::transaction transaction (wallet->second->store.environment, nullptr, false);
	...
	{
		ec = chratos::error_cli::unknown_command;
	}
