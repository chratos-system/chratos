...
TEST (node, block_store_path_failure)
{
	...
	auto node (std::make_shared<chratos::node> (init, *service, 0, path, alarm, logging, work));
...
TEST (node, password_fanout)
{
	...
	auto node (std::make_shared<chratos::node> (init, *service, path, alarm, config, work));
...
TEST (node, balance)
{
	...
	ASSERT_EQ (std::numeric_limits<chratos::uint128_t>::max (), system.nodes[0]->ledger.account_balance (transaction, chratos::test_genesis_key.pub));
}
...
TEST (node, send_unkeyed)
{
	...
	system.wallet (0)->insert_adhoc (chratos::test_genesis_key.prv);
	system.wallet (0)->store.password.value_set (chratos::keypair ().prv);
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (chratos::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
}
...
TEST (node, send_self)
{
	...
	ASSERT_EQ (std::numeric_limits<chratos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (chratos::test_genesis_key.pub));
}
...
TEST (node, send_single_observing_peer)
{
	...
	ASSERT_TRUE (system.nodes[0]->balance (key2.pub).is_zero ());
	system.deadline_set (10s);
	while (std::any_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<chratos::node> const & node_a) { return node_a->balance (key2.pub).is_zero (); }))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
{
	chratos::system system (24000, 10);
	chratos::keypair key2;
	system.wallet (0)->insert_adhoc (chratos::test_genesis_key.prv);
	system.wallet (1)->insert_adhoc (key2.prv);
	ASSERT_NE (nullptr, system.wallet (0)->send_action (chratos::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
...
TEST (node, send_out_of_order)
{
	...
	chratos::send_block send1 (genesis.hash (), key2.pub, std::numeric_limits<chratos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ()));
	chratos::send_block send2 (send1.hash (), key2.pub, std::numeric_limits<chratos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number () * 2, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (send1.hash ()));
	chratos::send_block send3 (send2.hash (), key2.pub, std::numeric_limits<chratos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number () * 3, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (send2.hash ()));
	system.nodes[0]->process_active (std::unique_ptr<chratos::block> (new chratos::send_block (send3)));
	system.nodes[0]->process_active (std::unique_ptr<chratos::block> (new chratos::send_block (send2)));
	system.nodes[0]->process_active (std::unique_ptr<chratos::block> (new chratos::send_block (send1)));
	system.deadline_set (10s);
	while (std::any_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<chratos::node> const & node_a) { return node_a->balance (chratos::test_genesis_key.pub) != chratos::genesis_amount - system.nodes[0]->config.receive_minimum.number () * 3; }))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, quick_confirm)
{
	chratos::system system (24000, 1);
	chratos::keypair key;
	chratos::block_hash previous (system.nodes[0]->latest (chratos::test_genesis_key.pub));
	auto genesis_start_balance (system.nodes[0]->balance (chratos::test_genesis_key.pub));
	system.wallet (0)->insert_adhoc (key.prv);
	system.wallet (0)->insert_adhoc (chratos::test_genesis_key.prv);
	auto send (std::make_shared<chratos::send_block> (previous, key.pub, system.nodes[0]->config.online_weight_minimum.number () + 1, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (previous)));
	...
	ASSERT_EQ (system.nodes[0]->balance (chratos::test_genesis_key.pub), system.nodes[0]->config.online_weight_minimum.number () + 1);
...
TEST (node, node_receive_quorum)
{
	...
	chratos::block_hash previous (system.nodes[0]->latest (chratos::test_genesis_key.pub));
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (std::make_shared<chratos::send_block> (previous, key.pub, chratos::genesis_amount - chratos::Gchr_ratio, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (previous)));
	system.nodes[0]->process_active (send);
	system.deadline_set (10s);
	...
	while (!done)
	{
		...
		done = info->announcements > chratos::active_transactions::announcement_min;
		ASSERT_NO_ERROR (system.poll ());
	}
	chratos::system system2 (24001, 1);
	system2.wallet (0)->insert_adhoc (chratos::test_genesis_key.prv);
...
TEST (node, receive_gap)
{
	...
	auto block (std::make_shared<chratos::send_block> (5, 1, 2, chratos::keypair ().prv, 4, 0));
...
{
	chratos::system system (24000, 1);
	std::array<chratos::endpoint, 8> endpoints;
	endpoints.fill (chratos::endpoint (boost::asio::ip::address_v6::loopback (), 24000));
	endpoints[0] = chratos::endpoint (boost::asio::ip::address_v6::loopback (), 24001);
...
TEST (node, search_pending_multiple)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (chratos::test_genesis_key.pub, key3.pub, system.nodes[0]->config.receive_minimum.number ()));
	system.deadline_set (10s);
...
TEST (node, unlock_search)
{
	chratos::system system (24000, 1);
	chratos::keypair key2;
	chratos::uint128_t balance (system.nodes[0]->balance (chratos::test_genesis_key.pub));
	{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (chratos::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
	system.deadline_set (10s);
	while (system.nodes[0]->balance (chratos::test_genesis_key.pub) == balance)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	system.wallet (0)->insert_adhoc (key2.prv);
...
TEST (node, connect_after_junk)
{
	...
	system.deadline_set (10s);
	while (system.nodes[0]->stats.count (chratos::stat::type::error) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	node1->start ();
...
TEST (node, working)
{
	auto path (chratos::working_path ());
...
{
	auto path1 (chratos::unique_path ());
	auto path2 (chratos::unique_path ());
	chratos::logging logging1;
	logging1.init (path1);
	chratos::logging logging2;
	logging2.init (path2);
...
{
	chratos::system system (24000, 1);
	auto price1 (system.nodes[0]->price (chratos::Gchr_ratio, 1));
	ASSERT_EQ (chratos::node::price_max * 100.0, price1);
	auto price2 (system.nodes[0]->price (chratos::Gchr_ratio * int(chratos::node::free_cutoff + 1), 1));
	ASSERT_EQ (0, price2);
	auto price3 (system.nodes[0]->price (chratos::Gchr_ratio * int(chratos::node::free_cutoff + 2) / 2, 1));
	ASSERT_EQ (chratos::node::price_max * 100.0 / 2, price3);
	auto price4 (system.nodes[0]->price (chratos::Gchr_ratio * int(chratos::node::free_cutoff) * 2, 1));
	ASSERT_EQ (0, price4);
}
...
TEST (node_config, serialization)
{
	...
	chratos::logging logging1;
	logging1.init (path);
	chratos::node_config config1 (100, logging1);
	...
	chratos::node_config config2 (50, logging2);
...
TEST (node_config, v1_v2_upgrade)
{
	...
	tree.put ("receive_minimum", chratos::amount (0).to_string_dec ());
...
TEST (node_config, v2_v3_upgrade)
{
	...
	config1.deserialize_json (upgraded, tree);
	//ASSERT_EQ (chratos::uint128_union (0).to_string_dec (), tree.get<std::string> ("inactive_supply"));
...
TEST (node, confirm_locked)
{
	...
	auto block (std::make_shared<chratos::send_block> (0, 0, 0, chratos::keypair ().prv, 0, 0));
...
{
	std::weak_ptr<chratos::node> node0;
	{
		chratos::system system (24000, 1);
		...
		chratos::keypair key2;
		auto send2 (std::make_shared<chratos::send_block> (genesis.hash (), key2.pub, chratos::genesis_amount - 100, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
		...
		node1.block_processor.flush ();
		auto existing1 (election->last_votes.find (chratos::test_genesis_key.pub));
...
TEST (node, fork_keep)
{
	...
	auto send1 (std::make_shared<chratos::send_block> (genesis.hash (), key1.pub, chratos::genesis_amount - 100, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	auto send2 (std::make_shared<chratos::send_block> (genesis.hash (), key2.pub, chratos::genesis_amount - 100, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	node1.process_active (send1);
	node1.block_processor.flush ();
...
TEST (node, fork_flip)
{
	...
	chratos::genesis genesis;
	auto send1 (std::make_shared<chratos::send_block> (genesis.hash (), key1.pub, chratos::genesis_amount - 100, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	chratos::publish publish1 (send1);
	chratos::keypair key2;
	auto send2 (std::make_shared<chratos::send_block> (genesis.hash (), key2.pub, chratos::genesis_amount - 100, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	chratos::publish publish2 (send2);
...
TEST (node, fork_multi_flip)
{
	...
	auto send2 (std::make_shared<chratos::send_block> (genesis.hash (), key2.pub, chratos::genesis_amount - 100, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	chratos::publish publish2 (send2);
	auto send3 (std::make_shared<chratos::send_block> (publish2.block->hash (), key2.pub, chratos::genesis_amount - 100, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (publish2.block->hash ())));
	chratos::publish publish3 (send3);
	node1.process_message (publish1, node1.network.endpoint ());
	node1.block_processor.flush ();
...
TEST (node, fork_bootstrap_flip)
{
	...
	chratos::block_hash latest (system0.nodes[0]->latest (chratos::test_genesis_key.pub));
	chratos::keypair key1;
	auto send1 (std::make_shared<chratos::send_block> (latest, key1.pub, chratos::genesis_amount - chratos::Gchr_ratio, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system0.work.generate (latest)));
	chratos::keypair key2;
	auto send2 (std::make_shared<chratos::send_block> (latest, key2.pub, chratos::genesis_amount - chratos::Gchr_ratio, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system0.work.generate (latest)));
...
TEST (node, fork_open)
{
	...
	chratos::keypair key1;
	chratos::genesis genesis;
	auto send1 (std::make_shared<chratos::send_block> (genesis.hash (), key1.pub, 0, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	chratos::publish publish1 (send1);
	node1.process_message (publish1, node1.network.endpoint ());
	node1.block_processor.flush ();
	auto open1 (std::make_shared<chratos::open_block> (publish1.block->hash (), 1, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
	chratos::publish publish2 (open1);
	node1.process_message (publish2, node1.network.endpoint ());
	node1.block_processor.flush ();
	auto open2 (std::make_shared<chratos::open_block> (publish1.block->hash (), 2, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
	chratos::publish publish3 (open2);
...
TEST (node, fork_open_flip)
{
	...
	chratos::keypair key1;
	chratos::genesis genesis;
	chratos::keypair rep1;
	chratos::keypair rep2;
	auto send1 (std::make_shared<chratos::send_block> (genesis.hash (), key1.pub, chratos::genesis_amount - 1, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	node1.process_active (send1);
	node2.process_active (send1);
	// We should be keeping this block
	auto open1 (std::make_shared<chratos::open_block> (send1->hash (), rep1.pub, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
	// This block should be evicted
	auto open2 (std::make_shared<chratos::open_block> (send1->hash (), rep2.pub, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
	...
	ASSERT_EQ (chratos::genesis_amount - 1, winner.first);
...
TEST (node, coherent_observer)
{
	chratos::system system (24000, 1);
	auto & node1 (*system.nodes[0]);
	node1.observers.blocks.add ([&node1](std::shared_ptr<chratos::block> block_a, chratos::account const &, chratos::uint128_t const &, bool) {
		auto transaction (node1.store.tx_begin ());
		ASSERT_TRUE (node1.store.block_exists (transaction, block_a->hash ()));
	});
	system.wallet (0)->insert_adhoc (chratos::test_genesis_key.prv);
	chratos::keypair key;
	system.wallet (0)->send_action (chratos::test_genesis_key.pub, key.pub, 1);
}
...
TEST (node, fork_no_vote_quorum)
{
	...
	system.wallet (0)->send_action (chratos::test_genesis_key.pub, key4, chratos::genesis_amount / 4);
	...
	auto block (system.wallet (0)->send_action (chratos::test_genesis_key.pub, key1, node1.config.receive_minimum.number ()));
	...
	chratos::state_block send1 (chratos::test_genesis_key.pub, block->hash (), chratos::test_genesis_key.pub, (chratos::genesis_amount / 4) - (node1.config.receive_minimum.number () * 2), key1, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (block->hash ()));
	ASSERT_EQ (chratos::process_result::progress, node1.process (send1).code);
	ASSERT_EQ (chratos::process_result::progress, node2.process (send1).code);
	ASSERT_EQ (chratos::process_result::progress, node3.process (send1).code);
	auto key2 (system.wallet (2)->deterministic_insert ());
	auto send2 (std::make_shared<chratos::send_block> (block->hash (), key2, (chratos::genesis_amount / 4) - (node1.config.receive_minimum.number () * 2), chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (block->hash ())));
	...
	auto vote (std::make_shared<chratos::vote> (key1, key3, 0, send2));
	...
	while (node3.stats.count (chratos::stat::type::message, chratos::stat::detail::confirm_ack, chratos::stat::dir::in) < 3)
	{
		system.poll ();
	}
	ASSERT_TRUE (node1.latest (chratos::test_genesis_key.pub) == send1.hash ());
	ASSERT_TRUE (node2.latest (chratos::test_genesis_key.pub) == send1.hash ());
	ASSERT_TRUE (node3.latest (chratos::test_genesis_key.pub) == send1.hash ());
}
...
TEST (node, DISABLED_fork_pre_confirm)
{
	...
	system.deadline_set (30s);
	auto block0 (system.wallet (0)->send_action (chratos::test_genesis_key.pub, key1.pub, chratos::genesis_amount / 3));
	ASSERT_NE (nullptr, block0);
	while (node0.balance (key1.pub) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto block1 (system.wallet (0)->send_action (chratos::test_genesis_key.pub, key2.pub, chratos::genesis_amount / 3));
	...
	auto block2 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, node0.latest (chratos::test_genesis_key.pub), key3.pub, node0.balance (chratos::test_genesis_key.pub), 0, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	auto block3 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, node0.latest (chratos::test_genesis_key.pub), key4.pub, node0.balance (chratos::test_genesis_key.pub), 0, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	...
	while (!done)
	{
		done |= node0.latest (chratos::test_genesis_key.pub) == block2->hash () && node1.latest (chratos::test_genesis_key.pub) == block2->hash () && node2.latest (chratos::test_genesis_key.pub) == block2->hash ();
		done |= node0.latest (chratos::test_genesis_key.pub) == block3->hash () && node1.latest (chratos::test_genesis_key.pub) == block3->hash () && node2.latest (chratos::test_genesis_key.pub) == block3->hash ();
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, DISABLED_fork_stale)
{
	...
	system1.wallet (0)->insert_adhoc (chratos::test_genesis_key.prv);
	chratos::system system2 (24001, 1);
	auto & node1 (*system1.nodes[0]);
	auto & node2 (*system2.nodes[0]);
	node2.bootstrap_initiator.bootstrap (node1.network.endpoint ());
	node2.peers.rep_response (node1.network.endpoint (), chratos::test_genesis_key.pub, chratos::genesis_amount);
	chratos::genesis genesis;
	chratos::keypair key1;
	chratos::keypair key2;
	auto send3 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, genesis.hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - chratos::Mchr_ratio, key1.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	...
	auto send1 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, send3->hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - 2 * chratos::Mchr_ratio, key1.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send1);
	auto send2 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, send3->hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - 2 * chratos::Mchr_ratio, key2.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send2);
	{
		auto transaction1 (node1.store.tx_begin (true));
		ASSERT_EQ (chratos::process_result::progress, node1.ledger.process (transaction1, *send1).code);
		auto transaction2 (node2.store.tx_begin (true));
		ASSERT_EQ (chratos::process_result::progress, node2.ledger.process (transaction2, *send2).code);
	}
	node1.process_active (send1);
	node1.process_active (send2);
	node2.process_active (send1);
...
TEST (node, broadcast_elected)
{
	...
	auto node1 (system.nodes[1]);
	auto node2 (system.nodes[2]);
	chratos::keypair rep_big;
	chratos::keypair rep_small;
	chratos::keypair rep_other;
	...
	//std::cerr << "Other: " << rep_other.pub.to_account () << std::endl;
	{
		...
		auto transaction1 (node1->store.tx_begin (true));
		auto transaction2 (node2->store.tx_begin (true));
		chratos::send_block fund_big (node0->ledger.latest (transaction0, chratos::test_genesis_key.pub), rep_big.pub, chratos::Gxrb_ratio * 5, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0);
		chratos::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
		chratos::send_block fund_small (fund_big.hash (), rep_small.pub, chratos::Gchr_ratio * 2, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0);
		chratos::open_block open_small (fund_small.hash (), rep_small.pub, rep_small.pub, rep_small.prv, rep_small.pub, 0);
		chratos::send_block fund_other (fund_small.hash (), rep_other.pub, chratos::Gchr_ratio * 1, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0);
		chratos::open_block open_other (fund_other.hash (), rep_other.pub, rep_other.pub, rep_other.prv, rep_other.pub, 0);
		...
		ASSERT_EQ (chratos::process_result::progress, node0->ledger.process (transaction0, fund_big).code);
		ASSERT_EQ (chratos::process_result::progress, node1->ledger.process (transaction1, fund_big).code);
		ASSERT_EQ (chratos::process_result::progress, node2->ledger.process (transaction2, fund_big).code);
		ASSERT_EQ (chratos::process_result::progress, node0->ledger.process (transaction0, open_big).code);
		ASSERT_EQ (chratos::process_result::progress, node1->ledger.process (transaction1, open_big).code);
		ASSERT_EQ (chratos::process_result::progress, node2->ledger.process (transaction2, open_big).code);
		ASSERT_EQ (chratos::process_result::progress, node0->ledger.process (transaction0, fund_small).code);
		ASSERT_EQ (chratos::process_result::progress, node1->ledger.process (transaction1, fund_small).code);
		ASSERT_EQ (chratos::process_result::progress, node2->ledger.process (transaction2, fund_small).code);
		ASSERT_EQ (chratos::process_result::progress, node0->ledger.process (transaction0, open_small).code);
		ASSERT_EQ (chratos::process_result::progress, node1->ledger.process (transaction1, open_small).code);
		ASSERT_EQ (chratos::process_result::progress, node2->ledger.process (transaction2, open_small).code);
		ASSERT_EQ (chratos::process_result::progress, node0->ledger.process (transaction0, fund_other).code);
		ASSERT_EQ (chratos::process_result::progress, node1->ledger.process (transaction1, fund_other).code);
		ASSERT_EQ (chratos::process_result::progress, node2->ledger.process (transaction2, fund_other).code);
		ASSERT_EQ (chratos::process_result::progress, node0->ledger.process (transaction0, open_other).code);
		ASSERT_EQ (chratos::process_result::progress, node1->ledger.process (transaction1, open_other).code);
		ASSERT_EQ (chratos::process_result::progress, node2->ledger.process (transaction2, open_other).code);
	}
	...
	auto fork0 (std::make_shared<chratos::send_block> (node2->latest (chratos::test_genesis_key.pub), rep_small.pub, 0, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	node0->work_generate_blocking (*fork0);
	node0->process_active (fork0);
	node1->process_active (fork0);
	auto fork1 (std::make_shared<chratos::send_block> (node2->latest (chratos::test_genesis_key.pub), rep_big.pub, 0, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
...
TEST (node, rep_self_vote)
{
	...
	{
		auto transaction0 (node0->store.tx_begin (true));
		chratos::send_block fund_big (node0->ledger.latest (transaction0, chratos::test_genesis_key.pub), rep_big.pub, chratos::uint128_t ("0xb0000000000000000000000000000000"), chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0);
		chratos::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
		node0->work_generate_blocking (fund_big);
		node0->work_generate_blocking (open_big);
		ASSERT_EQ (chratos::process_result::progress, node0->ledger.process (transaction0, fund_big).code);
	...
	system.wallet (0)->insert_adhoc (rep_big.prv);
	system.wallet (0)->insert_adhoc (chratos::test_genesis_key.prv);
	auto block0 (std::make_shared<chratos::send_block> (node0->latest (chratos::test_genesis_key.pub), rep_big.pub, chratos::uint128_t ("0x60000000000000000000000000000000"), chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	node0->work_generate_blocking (*block0);
	ASSERT_EQ (chratos::process_result::progress, node0->process (*block0).code);
	...
	ASSERT_NE (rep_votes.end (), rep_votes.find (chratos::test_genesis_key.pub));
...
TEST (node, DISABLED_bootstrap_no_publish)
{
	...
	chratos::send_block send0 (system0.nodes[0]->latest (chratos::test_genesis_key.pub), key0.pub, 500, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0);
	{
		auto transaction (node0->store.tx_begin (true));
		ASSERT_EQ (chratos::process_result::progress, system0.nodes[0]->ledger.process (transaction, send0).code);
	}
...
{
	chratos::system system0 (24000, 2);
	system0.wallet (0)->insert_adhoc (chratos::test_genesis_key.prv);
	auto node0 (system0.nodes[0]);
	auto node1 (system0.nodes[1]);
	chratos::keypair key0;
	chratos::send_block send0 (system0.nodes[0]->latest (chratos::test_genesis_key.pub), key0.pub, chratos::genesis_amount - 500, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0);
	chratos::open_block open0 (send0.hash (), 1, key0.pub, key0.prv, key0.pub, 0);
	chratos::open_block open1 (send0.hash (), 2, key0.pub, key0.prv, key0.pub, 0);
	node0->work_generate_blocking (send0);
	...
	node0->work_generate_blocking (open1);
	{
		...
		ASSERT_EQ (chratos::process_result::progress, node0->ledger.process (transaction0, send0).code);
		ASSERT_EQ (chratos::process_result::progress, node1->ledger.process (transaction1, send0).code);
		// They disagree about open0/open1
		ASSERT_EQ (chratos::process_result::progress, node0->ledger.process (transaction0, open0).code);
		ASSERT_EQ (chratos::process_result::progress, node1->ledger.process (transaction1, open1).code);
	}
	ASSERT_FALSE (node1->bootstrap_initiator.in_progress ());
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint ());
	ASSERT_TRUE (node1->active.roots.empty ());
...
TEST (node, DISABLED_unconfirmed_send)
{
	...
	wallet0->insert_adhoc (chratos::test_genesis_key.prv);
	auto send1 (wallet0->send_action (chratos::genesis_account, key0.pub, 2 * chratos::Mchr_ratio));
	system.deadline_set (10s);
	while (node1.balance (key0.pub) != 2 * chratos::Mchr_ratio || node1.bootstrap_initiator.in_progress ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto latest (node1.latest (key0.pub));
	chratos::state_block send2 (key0.pub, latest, chratos::genesis_account, chratos::Mchr_ratio, chratos::genesis_account, key0.prv, key0.pub, node0.work_generate_blocking (latest));
	{
	...
	auto send3 (wallet1->send_action (key0.pub, chratos::genesis_account, chratos::Mchr_ratio));
	system.deadline_set (10s);
	while (node0.balance (chratos::genesis_account) != chratos::genesis_amount)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, rep_list)
{
	...
	wallet0->send_action (chratos::test_genesis_key.pub, key1.pub, chratos::Mchr_ratio);
...
TEST (node, no_voting)
{
	...
	ASSERT_EQ (0, node1.stats.count (chratos::stat::type::message, chratos::stat::detail::confirm_ack, chratos::stat::dir::in));
}
...
TEST (node, vote_replay)
{
	chratos::system system (24000, 2);
	chratos::keypair key;
	auto open (std::make_shared<chratos::open_block> (0, 1, key.pub, key.prv, key.pub, 0));
	system.nodes[0]->work_generate_blocking (*open);
	for (auto i (0); i < 11000; ++i)
	{
		auto transaction (system.nodes[1]->store.tx_begin ());
		auto vote (system.nodes[1]->store.vote_generate (transaction, chratos::test_genesis_key.pub, chratos::test_genesis_key.prv, open));
	}
	{
		auto transaction (system.nodes[0]->store.tx_begin ());
		std::lock_guard<std::mutex> lock (boost::polymorphic_downcast<chratos::mdb_store *> (system.nodes[0]->store_impl.get ())->cache_mutex);
		auto vote (system.nodes[0]->store.vote_current (transaction, chratos::test_genesis_key.pub));
		ASSERT_EQ (nullptr, vote);
	}
	system.wallet (0)->insert_adhoc (chratos::test_genesis_key.prv);
	auto block (system.wallet (0)->send_action (chratos::test_genesis_key.pub, key.pub, chratos::Gchr_ratio));
	ASSERT_NE (nullptr, block);
	auto done (false);
...
TEST (node, balance_observer)
{
	...
	chratos::keypair key;
	node1.observers.account_balance.add ([&key, &balances](chratos::account const & account_a, bool is_pending) {
		if (key.pub == account_a && is_pending)
		{
			balances++;
		}
		else if (chratos::test_genesis_key.pub == account_a && !is_pending)
		{
			balances++;
		}
	});
...
TEST (node, stat_counting)
{
	chratos::system system (24000, 1);
	auto & node1 (*system.nodes[0]);
	node1.stats.add (chratos::stat::type::ledger, chratos::stat::dir::in, 1);
	node1.stats.add (chratos::stat::type::ledger, chratos::stat::dir::in, 5);
	node1.stats.inc (chratos::stat::type::ledger, chratos::stat::dir::in);
	node1.stats.inc (chratos::stat::type::ledger, chratos::stat::detail::send, chratos::stat::dir::in);
	node1.stats.inc (chratos::stat::type::ledger, chratos::stat::detail::send, chratos::stat::dir::in);
	node1.stats.inc (chratos::stat::type::ledger, chratos::stat::detail::receive, chratos::stat::dir::in);
	ASSERT_EQ (10, node1.stats.count (chratos::stat::type::ledger, chratos::stat::dir::in));
	ASSERT_EQ (2, node1.stats.count (chratos::stat::type::ledger, chratos::stat::detail::send, chratos::stat::dir::in));
	ASSERT_EQ (1, node1.stats.count (chratos::stat::type::ledger, chratos::stat::detail::receive, chratos::stat::dir::in));
}
...
TEST (node, block_confirm)
{
	...
	auto send1 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, genesis.hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - chratos::Gxrb_ratio, key.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.nodes[0]->work_generate_blocking (genesis.hash ())));
	...
	auto send2 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, send1->hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - chratos::Gxrb_ratio * 2, key.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.nodes[0]->work_generate_blocking (send1->hash ())));
	{
		auto transaction (system.nodes[0]->store.tx_begin (true));
		ASSERT_EQ (chratos::process_result::progress, system.nodes[0]->ledger.process (transaction, *send2).code);
	}
	{
		auto transaction (system.nodes[1]->store.tx_begin (true));
		ASSERT_EQ (chratos::process_result::progress, system.nodes[1]->ledger.process (transaction, *send2).code);
	}
...
TEST (node, block_arrival_size)
{
	chratos::system system (24000, 1);
	auto & node (*system.nodes[0]);
	auto time (std::chrono::steady_clock::now () - chratos::block_arrival::arrival_time_min - std::chrono::seconds (5));
	chratos::block_hash hash (0);
	for (auto i (0); i < chratos::block_arrival::arrival_size_min * 2; ++i)
	{
		node.block_arrival.arrival.insert (chratos::block_arrival_info { time, hash });
		++hash.qwords[0];
	}
	ASSERT_EQ (chratos::block_arrival::arrival_size_min * 2, node.block_arrival.arrival.size ());
	node.block_arrival.recent (0);
	ASSERT_EQ (chratos::block_arrival::arrival_size_min, node.block_arrival.arrival.size ());
}
...
TEST (node, confirm_quorum)
{
	...
	chratos::uint128_union new_balance (system.nodes[0]->config.online_weight_minimum.number () - chratos::Gxrb_ratio);
	auto send1 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, genesis.hash (), chratos::test_genesis_key.pub, new_balance, chratos::test_genesis_key.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.nodes[0]->work_generate_blocking (genesis.hash ())));
	{
		auto transaction (system.nodes[0]->store.tx_begin (true));
		ASSERT_EQ (chratos::process_result::progress, system.nodes[0]->ledger.process (transaction, *send1).code);
	}
	system.wallet (0)->send_action (chratos::test_genesis_key.pub, chratos::test_genesis_key.pub, new_balance.number ());
	system.deadline_set (10s);
...
TEST (node, vote_republish)
{
	...
	chratos::genesis genesis;
	chratos::send_block send1 (genesis.hash (), key2.pub, std::numeric_limits<chratos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ()));
	chratos::send_block send2 (genesis.hash (), key2.pub, std::numeric_limits<chratos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number () * 2, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ()));
	system.nodes[0]->process_active (std::unique_ptr<chratos::block> (new chratos::send_block (send1)));
	...
	system.nodes[0]->active.publish (std::unique_ptr<chratos::block> (new chratos::send_block (send2)));
	auto vote (std::make_shared<chratos::vote> (chratos::test_genesis_key.pub, chratos::test_genesis_key.prv, 0, std::unique_ptr<chratos::block> (new chratos::send_block (send2))));
...
TEST (node, vote_by_hash_republish)
{
	...
	system.nodes[0]->active.publish (std::unique_ptr<chratos::block> (new chratos::send_block (send2)));
	std::vector<chratos::block_hash> vote_blocks;
	vote_blocks.push_back (send2.hash ());
	auto vote (std::make_shared<chratos::vote> (chratos::test_genesis_key.pub, chratos::test_genesis_key.prv, 0, vote_blocks));
	ASSERT_TRUE (system.nodes[0]->active.active (send1));
	ASSERT_TRUE (system.nodes[1]->active.active (send1));
	system.nodes[0]->vote_processor.vote (vote, system.nodes[0]->network.endpoint ());
	while (!system.nodes[0]->block (send2.hash ()))
...
TEST (node, vote_by_hash_epoch_block_republish)
{
	...
	chratos::genesis genesis;
	chratos::send_block send1 (genesis.hash (), key2.pub, std::numeric_limits<chratos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ()));
	chratos::state_block epoch1 (chratos::genesis_account, genesis.hash (), chratos::genesis_account, chratos::genesis_amount, system.nodes[0]->ledger.epoch_link, epoch_signer.prv, epoch_signer.pub, system.work.generate (genesis.hash ()));
	system.nodes[0]->process_active (std::unique_ptr<chratos::block> (new chratos::send_block (send1)));
	system.deadline_set (5s);
	while (!system.nodes[1]->block (send1.hash ()))
	{
	...
	system.nodes[0]->active.publish (std::unique_ptr<chratos::block> (new chratos::state_block (epoch1)));
	std::vector<chratos::block_hash> vote_blocks;
...
TEST (node, fork_invalid_block_signature)
{
	...
	chratos::send_block send1 (genesis.hash (), key2.pub, std::numeric_limits<chratos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ()));
	chratos::send_block send2 (genesis.hash (), key2.pub, std::numeric_limits<chratos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number () * 2, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, system.work.generate (genesis.hash ()));
	chratos::send_block send2_corrupt (send2);
	send2_corrupt.signature = chratos::signature (123);
	system.nodes[0]->process_active (std::unique_ptr<chratos::block> (new chratos::send_block (send1)));
	system.deadline_set (5s);
	...
	auto vote (std::make_shared<chratos::vote> (chratos::test_genesis_key.pub, chratos::test_genesis_key.prv, 0, std::make_unique<chratos::send_block> (send2)));
	auto vote_corrupt (std::make_shared<chratos::vote> (chratos::test_genesis_key.pub, chratos::test_genesis_key.prv, 0, std::make_unique<chratos::send_block> (send2_corrupt)));
...
TEST (node, fork_invalid_block_signature_vote_by_hash)
{
	...
	system.nodes[0]->active.publish (std::make_unique<chratos::send_block> (send2_corrupt));
	ASSERT_NO_ERROR (system.poll ());
	system.nodes[0]->active.publish (std::make_unique<chratos::send_block> (send2));
	std::vector<chratos::block_hash> vote_blocks;
	vote_blocks.push_back (send2.hash ());
	auto vote (std::make_shared<chratos::vote> (chratos::test_genesis_key.pub, chratos::test_genesis_key.prv, 0, vote_blocks));
	{
...
TEST (node, block_processor_signatures)
{
	...
	chratos::block_hash latest (system0.nodes[0]->latest (chratos::test_genesis_key.pub));
	chratos::keypair key1;
	auto send1 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, latest, chratos::test_genesis_key.pub, chratos::genesis_amount - chratos::Gxrb_ratio, key1.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send1);
	chratos::keypair key2;
	auto send2 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, send1->hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - 2 * chratos::Gxrb_ratio, key2.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send2);
	chratos::keypair key3;
	auto send3 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, send2->hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - 3 * chratos::Gxrb_ratio, key3.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send3);
	// Invalid signature bit
	auto send4 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, send3->hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - 4 * chratos::Gxrb_ratio, key3.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send4);
	send4->signature.bytes[32] ^= 0x1;
	// Invalid signature bit (force)
	auto send5 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, send3->hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - 5 * chratos::Gxrb_ratio, key3.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	...
	auto receive1 (std::make_shared<chratos::state_block> (key1.pub, 0, chratos::test_genesis_key.pub, chratos::Gxrb_ratio, send1->hash (), key1.prv, key1.pub, 0));
	node1.work_generate_blocking (*receive1);
	auto receive2 (std::make_shared<chratos::state_block> (key2.pub, 0, chratos::test_genesis_key.pub, chratos::Gxrb_ratio, send2->hash (), key2.prv, key2.pub, 0));
	node1.work_generate_blocking (*receive2);
	// Invalid private key
	auto receive3 (std::make_shared<chratos::state_block> (key3.pub, 0, chratos::test_genesis_key.pub, chratos::Gxrb_ratio, send3->hash (), key2.prv, key3.pub, 0));
...
TEST (node, block_processor_reject_state)
{
	...
	auto & node (*system.nodes[0]);
	chratos::genesis genesis;
	auto send1 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, genesis.hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - chratos::Gxrb_ratio, chratos::test_genesis_key.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
	...
	ASSERT_FALSE (node.ledger.block_exists (send1->hash ()));
	auto send2 (std::make_shared<chratos::state_block> (chratos::test_genesis_key.pub, genesis.hash (), chratos::test_genesis_key.pub, chratos::genesis_amount - 2 * chratos::Gxrb_ratio, chratos::test_genesis_key.pub, chratos::test_genesis_key.prv, chratos::test_genesis_key.pub, 0));
